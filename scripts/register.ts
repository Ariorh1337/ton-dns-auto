...
const REGISTER_VALUE = toNano('0.05');
...
if (!subdomain || !ownerAddress) {
    throw new Error('Использование: ts-node scripts/register.ts <subdomain> <owner_address>');
}

main(subdomain, ownerAddress).catch((error) => {
    console.error('Error during register:', error);
});

async function main(subdomain: string, ownerAddress: any) {
